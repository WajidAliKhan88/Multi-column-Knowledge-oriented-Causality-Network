...
class TKC(nn.Module):
    ...
    # Three knowledge-oriented channels/columns(TKC) Network...
        self.tkc = TemporalConvNet(n_emb, n_channel, kernel_size=n_kernel, dropout=dropout['layer'])
    ...
    # Pass input x_emb.transpose to tkc network...
        y = self.tkc(x_emb.transpose(2, 1))
        y = y.transpose(1, 2)
        for i in range(self.n_block):
