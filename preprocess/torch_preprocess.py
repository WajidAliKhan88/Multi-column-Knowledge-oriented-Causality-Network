...
def stat_altlex(eng_sentences, sim_sentences, labels):
    c_alt, nc_alt = [], [] # causal and non-causal altlex...
    ...
    c_alt_set = set(c_alt)  # convert it into set format..
    nc_alt_set = set(nc_alt)

    co_alt_set = c_alt_set.intersection(nc_alt_set) # common alt_set...
    co_in_c, co_in_nc = 0, 0
# This loop is used, if c is in co_alt_set, then co_in_c = co_in_c + 1....
...
def seg_length(sentences): # segment length...
    seg_len = []
    for sen in sentences:
        seg_len.append((len(sen[0]), len(sen[1]), len(sen[2])))
    return seg_len

# This function is to check either the sentence is null or not...
def check_null(sen):
    flag = False
    if len(sen) == 3:
        # if len(sen[0]) > 0:
...
def preprocess_train(file_path, file_name, data_type, is_build=False):
    ...
    for line in lines:                  # take one line...
    ...
    for label, eng, sim in zip(labels, seg_eng_filtered, seg_sim_filtered):
        total += 1
        pre, mid, cur, flag = check_null(eng) # for english...
        if flag:
            print(total)
        ...
        total += 1
        pre, mid, cur, flag = check_null(sim) # simple wiki...
        if flag:
            print(total)
        examples.append({'eid': total,
                         'tokens': pre + mid + cur,
...
def build_dict(data_path):
    ...
    with open(data_path, 'r', encoding='utf8') as fh:
        for line in fh:
            line = line.strip().split(' ')
            fredist = nltk.FreqDist(line)     # Frequency distribution...
            for localkey in fredist.keys():
                if localkey in dictionary.keys():
                    ...
                    dictionary[localkey] = fredist[localkey]  # Add current word frequency to dictionary
    return set(dictionary)

# saving the preprocess file...
