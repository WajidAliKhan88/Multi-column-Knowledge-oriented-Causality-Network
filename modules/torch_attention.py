...
class WordEmbedding(nn.Module):
    def __init__(self, vocab_size, num_units, zeros_pad=True, scale=True):
        ...
        Args:
          ...
          num_units: An int. Number of embedding hidden units. or dimensionality
        ...
        if self.zeros_pad:
            self.lookup_table.data[0, :].fill_(0) # 0, mean the first row, and :, mean the all column fill with zero.
...
# Query(Q), Key(K), and V(value)..._Proj mean weights for Q, K, and V...
...
class FeedForward(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # Residual connection...
        outputs += inputs    # outputs = outputs +inputs...
